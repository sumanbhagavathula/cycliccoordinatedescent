...
cd.objective_plot(bestlambdaCV, x_train, y_train, betas_cyclic = cyclicbetas, betas_random = [])

test_mean_square_error = cd.computescore(cyclicbetas[-1],x_test,y_test)

print('test mean square error with my algorithm: ' + str(test_mean_square_error))

#run cross validation using sklearn
lassoCV = LassoCV(eps=0.0001,max_iter=1000,cv=10,selection='cyclic',random_state=0,fit_intercept=False,normalize=False)
...
lasso = Lasso(alpha=lassoCV.alpha_,max_iter=1000,selection='cyclic',random_state=0,fit_intercept=False,normalize=False)
